...
des = ["拉萨贡嘎国际机场", "大昭寺", "西藏博物馆", "布达拉宫", "桑耶寺", "浪卡子镇"]
...
def get_weather_forecast(adcode, needed_day, days=3):
    """
    获取未来天气预报
    :param adcode: 区域编码
    ...
    # 获取未来日期
    future_dates = [(datetime.now() + timedelta(days = days)).strftime("%Y-%m-%d")]
...
def save_weather_to_kml(weatherset, gpsset, output_file):
    ...
    for i in range(len(weatherset)):
        ...
        # 根据天气类型设置图标样式
        point.style.iconstyle.icon.href = "/Users/zhuhongjin/Downloads/11.png"
...
if __name__ == '__main__':
    ...
    for i in range(len(deslotation)-1):
        ...
        time.sleep(1)
    kml.save("day26_route.kml")
    needed_day = '2025-04-26'
    for i in range(len(adcode)):
        weather.append(get_weather_forecast(adcode[i], needed_day))
    print(weather)
    save_weather_to_kml(weather, deslotation,'day26_weather.kml')
